/**
 * Notion Integration - Connect Route
 *
 * POST /api/notion-integration/connect
  ...
  try {
    ...
    if (!isNotionConfigured()) {
      return NextResponse.json(
        { error: NOTION_ERROR_MESSAGES.notConfigured },
        { status: 503 }
      )
    }
    ...
    if (alreadyConnected) {
      return NextResponse.json(
        { error: NOTION_ERROR_MESSAGES.alreadyConnected },
        { status: 400 }
      )
    }
    ...
    console.error("[NotionConnect] Error:", error)
    return NextResponse.json(
